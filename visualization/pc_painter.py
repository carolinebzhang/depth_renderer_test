...
class PC_from_DEP(object):
    ...
    def get_point_cloud(self, depth_maps, cam_Ks, cam_RTs, rgb_imgs=None):
        ...
        # save the pcd to the disk 
        save_dir = './datasets/point_clouds'
        os.makedirs(save_dir, exist_ok=True)
        print(f'Saving pointclouds to path : {save_dir}')

        # np.savetxt(osp.join(save_dir, 'point_canonical.txt'), point_canonical)
        for i, point_cloud in enumerate(point_list_canonical):
            np.save(osp.join(save_dir, 'point_canonical_{:03d}.npy'.format(i)), point_cloud)
